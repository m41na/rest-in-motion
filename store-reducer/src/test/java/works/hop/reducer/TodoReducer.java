...
public class TodoReducer extends AbstractReducer<List<Todo>> {
    ...
    public List<Todo> reduce(State<List<Todo>> state, Action action) {
        switch (action.getType().get()) {
            case "ADD_TODO":
                List<Todo> added = state.apply(null);
                added.add((Todo) action.getBody());
                return added;
            case "REMOVE_TODO":
                String toRemove = (String) action.getBody();
                return state.apply(null).stream().filter(todo1 -> !todo1.task.equalsIgnoreCase(toRemove)).collect(Collectors.toList());
            case "COMPLETE_TODO":
    ...
    public Object compute(Action action) {
        switch (action.getType().get()) {
            ...
            case "TODO_BY_ID":
                ...
                return this.state().apply(null).stream().filter(r -> r.id == fetchId).findFirst().get();
            default:
                return null;
        }
    }
