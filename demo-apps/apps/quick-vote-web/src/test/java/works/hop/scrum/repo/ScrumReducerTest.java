...
import works.hop.scrum.config.RepoConfigTest;

import javax.sql.DataSource;
import java.io.IOException;
import java.util.Map;

import static works.hop.scrum.repo.ScrumActions.*;

@RunWith(SpringJUnit4ClassRunner.class)
@ContextConfiguration(classes = RepoConfigTest.class, loader = AnnotationConfigContextLoader.class)
...
public class ScrumReducerTest {
    ...
    public void test_RETRIEVE_SCRUM_ACTION() {
        String scrumId = "e8a7c326-7e42-469a-b0bd-e4f4e399077b";
    ...
    public void test_INVITE_PLAYER_ACTION() {
        ...
        PlayerJoin playerEntity = PlayerJoin.builder().player(player).scrumId(scrumId).build();
    ...
    public void test_EXIT_SCRUM_ACTION() {
        String scrumId = "7b987e2c-e969-4422-b416-961ca85b439d:steve:SCRUMS_COLLECTION";
        String player = "jane";
        PlayerExit playerExit = PlayerExit.builder().name(player).scrumId(scrumId).build();
        store.dispatchAsync(EXIT_SCRUM_ACTION.apply(playerExit), state -> {
            try {
                RecordEntity entity = (RecordEntity) state;
                Scrum result = mapper.readValue(entity.getValue(), Scrum.class);
                result.setResourceId(entity.getKey().getRecordId().toString());
