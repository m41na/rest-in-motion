...
public class ScrumReducer<S> extends AbstractReducer<S> {
    ...
    public S reduce(State<S> state, Action action) {
        switch (action.getType().get()) {
            ...
            case INVITE_PLAYER: {
                PlayerJoin playerJoin = (PlayerJoin) action.getBody();
                RecordEntity entity = ((JdbcState) state).fetch(playerJoin.getScrumId());
            ...
            case JOIN_SCRUM: {
                PlayerJoin joining = (PlayerJoin) action.getBody();
            ...
            case EXIT_SCRUM: {
                PlayerExit exiting = (PlayerExit) action.getBody();
